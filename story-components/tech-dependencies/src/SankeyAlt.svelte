<script lang="ts">
  ...
  function estimateTooltipHeight(vendorCount: number): number {
    return 40 + (vendorCount > 0 ? 16 + vendorCount * 19 : 0);
  }
  ...
  const tooltipContent = $derived(
    hoveredNode
      ? {
          ...
          chf: hoveredNode.totalCHF,
          vendors: hoveredNode.vendors,
        }
      : hoveredEdge
        ? { label: `${hoveredEdge.source.label} → ${hoveredEdge.target.label}`, chf: hoveredEdge.valueCHF, vendors: undefined }
        : null,
  );
  ...
  const tooltipStyle = $derived.by(() => {
    ...
    const vendorCount = tooltipContent.vendors?.length ?? 0;
    const maxW = vendorCount > 0 ? 220 : 180;
    const height = tooltipEl?.offsetHeight ?? estimateTooltipHeight(vendorCount);
    const gap = 12;
    const left = Math.min(Math.max(4, mouseX + 14), width - maxW);

    let top = mouseY + gap;
    if (top + height > svgHeight - 8) {
      top = mouseY - height - gap;
    }
    top = Math.max(8, top);
...
>
  ...
  {#if tooltipContent && tooltipStyle}
    <div
      bind:this={tooltipEl}
      class="tooltip"
      class:tooltip-wide={tooltipContent.vendors?.length}
      style="left: {tooltipStyle.left}px; top: {tooltipStyle.top}px; max-width: {tooltipStyle.maxW}px;"
    >
      ...
      {/if}
      {#if tooltipContent.vendors?.length}
        <ul class="tooltip-vendors">
          {#each tooltipContent.vendors as vendor}
            <li>
              <span class="vendor-name">{vendor.name}</span>
              <span class="vendor-chf">{formatCHF(vendor.chf)}</span>
            </li>
          {/each}
        </ul>
      {/if}
