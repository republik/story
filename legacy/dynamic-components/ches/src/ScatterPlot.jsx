...
class ScatterPlot extends Component {
  constructor (...args) {
    ...
    this.focus = (event) => {
      ...
      const withDistance = this.symbols.map(symbol => {
        return {
          symbol,
          distance: Math.sqrt(
            Math.pow(symbol.data.cx - focusX, 2) +
            Math.pow(symbol.data.cy - focusY, 2)
          ) - symbol.data.r
        }
      })
    ...
    this.willLeave = symbol => ({
      cx: symbol.data.cx,
      cy: symbol.data.cy,
      r: spring(0, scfg),
      textOpacity: spring(0, scfg)
    })
    this.willEnter = symbol => ({
      cx: symbol.data.cx,
      cy: symbol.data.cy,
      r: 0,
      textOpacity: 0
    })
    this.willUpdate = symbol => ({
      cx: spring(symbol.data.cx, scfg),
      cy: spring(symbol.data.cy, scfg),
      r: spring(symbol.data.r, scfg),
      textOpacity: spring(1, scfg)
    })
  }
  componentDidMount () {
  ...
  renderHover ({ width, height, xFormat, yFormat }) {
    ...
    const { cx, cy, r } = hover.sort((a, b) => ascending(a.data.cy, b.data.cy))[0].data
  ...
  render () {
    ...
    this.symbols = data.map((value, i) => {
      ...
      symbol.style = this.willUpdate(symbol)
    ...
    return (
      <div style={{ position: 'relative' }}>
        <svg width={width} height={height} ref={this.setContainerRef}>
          ...
          <TransitionMotion willEnter={this.willEnter} willLeave={this.willLeave} styles={this.symbols}>
            {symbols => (
              <Fragment>
                {symbols.map((symbol, i) => (
                  <circle key={symbol.key}
                    style={{ opacity }}
                    fill={color(colorAccessor(symbol.data.value))}
                    cx={symbol.style.cx}
                    cy={symbol.style.cy}
                    r={Math.max(0, symbol.style.r)} />
                ))}
                {(inlineLabel || inlineSecondaryLabel) && symbols
                  .filter(({ data: { value: { datum } } }) => datum[inlineLabel] || datum[inlineSecondaryLabel])
                  .map((symbol, i) => {
                    const { datum } = symbol.data.value
                    ...
                    let yOffset = 0
                    let xOffset = 0
                    const r = Math.max(0, symbol.style.r)
                    if (pos === 'left') {
                      textAnchor = 'end'
                      xOffset = -(r + 5)
                    }
                    ...
                    if (pos === 'top' || pos === 'bottom') {
                      yOffset = r + 5 + (primary && secondary ? 15 : 7)
                      if (pos === 'top') {
                        yOffset = -yOffset
                      }
                    }
                    ...
                    return (
                      <g
                        ...
                        style={{ opacity: symbol.style.textOpacity }}
                        textAnchor={textAnchor}
                        transform={`translate(${symbol.style.cx + xOffset},${symbol.style.cy + yOffset})`}>
